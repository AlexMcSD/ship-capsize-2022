...
def model(h,k,D): ## this returns, f, Df(xplus),Df(xminus),xplus,xminus: where xplus/minus are saddle points
    kx,ky = k,k
...
def eigen(h,k): #eigenvalues and eigen vectors and change of basis matrix
    ...
    center1minus = array([-k/2,-beta*k/(16*np.sqrt(h)),alpha/4,1 ])
    center2minus = array([ omega, beta*omega/(8*np.sqrt(h)),0,0])
    stableminus = array([-k/2 + sqrt(k**2 + beta)/2,-alpha*(-k+np.sqrt(k**2+beta))/(16*np.sqrt(h)),-beta/4,1])
    unstableminus = array([-k/2 - sqrt(k**2 + beta)/2,alpha*(k+np.sqrt(k**2+beta))/(16*np.sqrt(h)),-beta/4,1])
